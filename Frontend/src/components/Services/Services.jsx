    ...
    <div className="min-h-screen bg-gray-100 flex flex-col items-center py-16 px-4">
      ...
      <div className="grid gap-8 sm:grid-cols-2 lg:grid-cols-3 max-w-7xl w-full">
        {services.map((service, index) => (
          <div
            key={index}
            className="relative bg-white rounded-lg shadow-lg p-6 hover:shadow-xl transition-shadow duration-300 flex flex-col"
          >
            <div className="flex items-center mb-5">
              <div className={`flex items-center justify-center rounded-full w-24 h-24 bg-white shadow-md ${service.color} mr-4`}>
                {service.icon}
              </div>
            </div>
